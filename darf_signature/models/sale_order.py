...
class sign_sale_order(models.Model):
    ...
    def getDocumentMD5(self):
        check_fields = str(self.incoterm)+str(self.partner_id)+str(self.confirmation_date)
        return hashlib.md5(check_fields).hexdigest()
    
    @api.one
    def get_ethereum_addres(self):
        ethereum_address = self.env['ethereum.config'].search([])
        result_ethereum_dic = {}
        if ethereum_address:
            result_ethereum_dic.update({'ethereum_address':ethereum_address[0].ethereum_address,
                                        'ethereum_interface': ethereum_address[0].ethereum_interface,
                                        'address_node':ethereum_address[0].ethereum_node})
